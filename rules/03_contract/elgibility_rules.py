...
def inactive_policy():
    return Rule(when=Fact(of_type=ClaimContext, var='claim_context', 
        ...
        then=lambda ctx: insert(ctx, Action(str(uuid.uuid4()), 'NOACT', 
                                            ctx.claim_context.claim.id, 
                                            'deny', 'policy inactive', 0.00)))
...
def late_filing():
    return Rule(when=Fact(of_type=ClaimContext, var='claim_context',
                ...
                then=lambda ctx: insert(ctx, Action(str(uuid.uuid4()), 'LATFL', 
                                                    ctx.claim_context.claim.id, 
                                                    'deny', 'late filing', 0.00)))
...
def vin_mismatch():
    return Rule(when=Fact(of_type=ClaimContext, var='claim_context',
                ...
                then=lambda ctx: insert(ctx, Action(str(uuid.uuid4()), 'AUTMIS', 
                                                    ctx.claim_context.claim.id, 
                                                    'deny', 'Automobile not registred in policy', 0.00)))
@ruledef
