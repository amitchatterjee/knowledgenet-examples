...
def no_police_report():
    return Rule(id='no-police-report', repository='autoclaims', ruleset='002-validate',
        when=Condition(of_type=AdjClaim, matches_exp=lambda ctx,this: not this.claim.police_report 
                       and assign(ctx, adjclaim=this)),
        then=lambda ctx: insert(ctx, Action('id', ctx.adjclaim.claim.id, 'd', 'no-police-report', 0.00)))
