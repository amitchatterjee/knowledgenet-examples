...
def compute_collision_payment():
    ...
    def compute_collision_payment_rhs(ctx):
        lowest_estimate = min(ctx.adj.estimates, key=lambda est: est.amount)
    ...
    return Rule(run_once=True, order=2,
        when=[Fact(of_type=Adj, var='adj', matches=lambda ctx,this: this.claim.type == 'collision'), 
            ...
            Fact(of_type=BlueBook,
                 matches=lambda ctx,this: assign(ctx, bluebook_value=this.lookup(ctx.adj.automobile.make, ctx.adj.automobile.model,ctx.adj.automobile.year)))],
